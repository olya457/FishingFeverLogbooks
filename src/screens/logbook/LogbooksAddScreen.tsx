...
const LogbookAddScreen = () => {
  const navigation = useNavigation<any>();
  const insets = useSafeAreaInsets();
  const { width, height } = useWindowDimensions();
  const { addCatch } = useLogbookStore();
  ...
  const date = useMemo(() => new Date().toLocaleDateString('uk-UA'), []);
  ...
  const ui = useMemo(() => {
    ...
    return {
      hPad,
      cardW,
      topBar: insets.top + (vs ? 6 : sm ? 10 : 16),
      ...
      smallBtnW: vs ? 44 : sm ? 50 : 58,
    };
  }, [width, height, insets.top]);

  const getValue = useCallback(
    (k: FieldKey) => (k === 'fishType' ? fishType : k === 'description' ? description : location),
    [fishType, description, location]
  );

  const setValue = useCallback((k: FieldKey, v: string) => {
  ...
  const handleKey = useCallback(
    ...
    [activeField, getValue, isNumMode, isUpperCase, moveNextField, setValue]
  );
  ...
  const handleClearAll = useCallback(() => {
    ...
    setLocation('');
    setPhotoUri(null);
    setActiveField('fishType');
    setIsUpperCase(true);
    setIsNumMode(false);
  }, []);

  const handleSave = useCallback(() => {
    const nextFishType = fishType.trim();
    ...
    navigation.goBack();
  }, [addCatch, date, description, fishType, location, navigation, photoUri]);
      ...
      >
        ...
        >
          ...
          <View style={[styles.infoRow, { minHeight: ui.fieldH }]}>
            ...
            <Text style={[styles.dateTxt, { fontSize: ui.fieldFont }]}>{date}</Text>
          </View>
          ...
          <View style={[styles.bottomRow, { gap: 8 }]}>
            <TouchableOpacity
              ...
              style={[
                styles.delBtn,
                { width: ui.smallBtnW, height: ui.btnH, borderRadius: 10 },
              ]}
            >
              <Text style={{ fontSize: ui.emojiSz }}>🗑</Text>
            </TouchableOpacity>
...
export default LogbookAddScreen;